...
Prospect it, mine it, pan it, survey and handpick the rest, keep the paydirt.
...
`mask` blinds a dataset of items under opaque keys and `survey` is the machine judge over it. `handpick` is the tool a person uses
...
motherlode mask --dataset datasets/trajectories-v6 --out work/masked
# 3. score: a grader's JSON per key (a subagent, a person), or an API model over every key
motherlode survey --masked work/masked --key 3f2a9c1e --rater claude-code:opus --scores '{"D1": {"score": 1, "rationale": "..."}, ...}'
...
motherlode handpick --dataset datasets/trajectories-v6 --masked work/masked --context packet --rater zachary --out work/grade.html
# 5. validate, per dimension, with ground truth from the dataset as a third rater
motherlode prospect --dataset datasets/trajectories-v6 --human work/labels-zachary.jsonl --judge work/masked/judgments.jsonl --adjudication work/adjudicate.jsonl
# 6. the graded dataset, hashed to its source
motherlode paydirt --dataset datasets/trajectories-v6 --masked work/masked --human work/labels-zachary.jsonl --out datasets/trajectories-v6-graded
```
...
| `motherlode.survey` | `mask` and `survey`: the machine judge over an items dataset, blind, deterministic keys; a masked directory refuses a changed dataset or rubric. |
...
The `api` extra adds the Anthropic client for `survey --model`.
