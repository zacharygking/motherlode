"""Assay: the machine judge over an items dataset.

``pool`` writes every item's text under an opaque key with the rubric and grading instructions,
so a grader, a person or a model, cannot tell where an item came from. ``score`` records one
grader's scores for a key, or asks an API model for them, and appends dimensioned label rows to
``judgments.jsonl`` in the workspace. Nothing here knows what the items are about.

    motherlode assay pool  --dataset datasets/trajectories --out work/assay
    motherlode assay score --workspace work/assay --key 3f2a9c1e --rater claude-code:opus --scores '<json>'
    motherlode assay score --workspace work/assay --model claude-opus-5          # every unscored key
"""
